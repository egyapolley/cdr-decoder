...
def main():
    ...
    parser.add_argument('-f', '--format', nargs='?', choices=['json', 'simple'], default='simple', action='store',
                        help='output format')
    parser.add_argument('-src', '--source', action='store', default='raw_cdrs',
                        help='raw cdr file or directory to fetch raw cdr files')
    parser.add_argument('-dst', '--destination', action='store', default='decoded_cdrs',
                        help='directory where to write decoded cdr files')
        ...
        try:
            ...
            if dargs['print']:
                ...
                if dargs['inonefile']:
                    ...
                    for decoded_header, index, decoded_cdr, numRecord in ff.decodeit(dargs):
                        decoded_header += ','
                        if index != numRecord:
                            decoded_cdr += ','
                    ...
                    for decoded_header, index, decoded_cdr in ff.decodeit(dargs):
                        ...
                        with open(destination_file, 'w') as f:
                            f.write(decoded_header)
                        cdr_destination_file = os.path.join(dargs['destination'],
                                                            'cdr_index_' + str(index) + '_' + ff.name + '_decoded')
                        with open(cdr_destination_file, 'w') as f:
                            f.write(decoded_cdr)
            print('[{}] Done with file {}'.format(i, cdr_file))
        except Exception as e:
            print(e)
            print('[{}] Error with with file {}'.format(i, cdr_file))
